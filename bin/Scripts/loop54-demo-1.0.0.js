...
var Demo = {
    ...
    Keyup: function Keyup(e) {
        ...
        //arrowdown
            ...
            //enter
                    ...
                    if (selecteda.length > 0) {
                        ...
                        if (f) Demo.AddFilter(demoConfig.AutoCompleteFacetingParameter, f);
        ...
        if (Demo.AutoCompleteQueries.length == 0) {
            ...
            if (Demo.FetchingAutoComplete) return true;
            ...
            request.getResponse(demoConfig.Url, function (response) {
                ...
                if (Demo.RunningACRequests > 0) return;

                if (!response.success && demoConfig.DevMode) alert(response.errorMessage);
    ...
    CreateFacetedAutoCompleteHtml: function CreateFacetedAutoCompleteHtml(query, facet) {
    ...
    ShowAutoComplete: function ShowAutoComplete() {
        ...
        if (Demo.AutoCompleteQueries.length > 0) {
            ...
            if (!found) div.hide();else div.show();
        }
    },
    ...
    HighLightSuggestion: function HighLightSuggestion() {
        ...
        if (Demo.ActiveIndex < -1) Demo.ActiveIndex = -1;

        if (Demo.ActiveIndex > divs.length - 1) Demo.ActiveIndex = divs.length - 1;

        if (Demo.ActiveIndex == -1) return;

        $(divs[Demo.ActiveIndex]).addClass("selected");
    },

    SearchHighLighted: function SearchHighLighted() {

        if (Demo.ActiveIndex < 0) return;

        var divs = $("div#autocomplete div.suggestion").removeClass("active");

        if (Demo.ActiveIndex > divs.length - 1) return;
    ...
    Search: function Search(query, instant, preventReSearch, page) {
        ...
        if (!isContinuation) {
            ...
            if (!instant) $("div#autocomplete").hide();
        }
        ...
        if (preventReSearch) request.setValue("PreventReSearch", true);
        ...
        request.getResponse(demoConfig.Url, function (response) {
            ...
            if (!response.data.MakesSense) {
                ...
                if (response.data.DirectResults && response.data.DirectResults.length > 0) $("div#nosenseheader").append($("<span>The results below are approximate.</span>"));
                ...
                if (response.data.SpellingSuggestions && response.data.SpellingSuggestions.length > 0) {
                    ...
                    for (var i = 0; i < response.data.SpellingSuggestions.length; i++) {
                        $("div#spellingsuggestions").append($("<a/>").html(response.data.SpellingSuggestions[i].Key).data("query", response.data.SpellingSuggestions[i].Key).click(function () {
                            Demo.ClearFilters();
                            Demo.Search($(this).data("query"));
                        }));
                    }
            ...
            if (response.data.ReSearchQueryString) {
                ...
                var reSearchString = response.data.ReSearchQueryString instanceof Array ? response.data.ReSearchQueryString[0].Key ? response.data.ReSearchQueryString[0].Key : response.data.ReSearchQueryString[0].String : response.data.ReSearchQueryString;

                $("div#research").show().html("We assumed you meant \"" + reSearchString + "\". Can you blame us?<br /><br />Search instead for ").append($("<a />").html(query).click(function () {
                    Demo.ClearFilters();
            ...
            if (response.data.RelatedQueries && response.data.RelatedQueries.length > 0) {
                ...
                for (var i = 0; i < response.data.RelatedQueries.length; i++) {
                    ...
                    $("div#related").append($("<a/>").html(response.data.RelatedQueries[i].Key).data("query", response.data.RelatedQueries[i].Key).click(function () {
                        Demo.ClearFilters();
            ...
            if (response.data.DirectResults && response.data.DirectResults.length > 0) {
                ...
                if (!isContinuation) $("div#directresults").append($("<h2>We found " + response.data.DirectResults_TotalItems + " results</h2>"));
            ...
            if (response.data.RecommendedResults && response.data.RecommendedResults.length > 0) {
                ...
                if (!isContinuation) $("div#recommendedresults").append($("<h2>You might also like</h2>"));
                ...
                if (data && data.length > 0) {
                    ...
                    for (var j = 0; j < data.length; j++) {
                        ...
                        if (j == 5) {
                            ...
                            if (Demo.VisibleFilterDivs[demoConfig.Filters[i].Name]) div.show();
                        ...
                        div.append($("<a/>").html(data[j].Key + " (" + data[j].Value + ")").data("filterkey", demoConfig.Filters[i].RequestParameter).data("filtervalue", data[j].Key).click(function () {
    ...
    AddFilter: function AddFilter(requestParameter, value) {

        if (!Demo.Filters[requestParameter]) Demo.Filters[requestParameter] = [];

        Demo.Filters[requestParameter].push(value);
    },

    RemoveFilter: function RemoveFilter(requestParameter, value) {
    ...
    ShowEntity: function ShowEntity(entity, value) {
        ...
        $("<img/>").attr("src", Demo.ReplaceImageUrl(entity)).appendTo(div).on("error", function () {
            $(this).remove();
        });
        ...
        if (demoConfig.DevMode) hiddenDiv.show();
    },
    ...
    RenderEntity: function RenderEntity(element, entity, value) {
        ...
        if (demoConfig.ShowValues) div.attr("title", value);

        var a = $("<a/>").appendTo(div);
        var imgDiv = $("<div/>").appendTo(a);
        $("<img/>").attr("src", Demo.ReplaceImageUrl(entity)).appendTo(imgDiv).on("load", function () {

            if ($(this).width() > $(this).height()) $(this).css("width", "100%");else $(this).css("height", "100%");
        }).on("error", function () {
            $(this).remove();
        });
    ...
    GetEntityTitle: function GetEntityTitle(entity) {

        if (entity.Attributes[demoConfig.ProductTitleAttribute]) return entity.Attributes[demoConfig.ProductTitleAttribute][0];
    ...
    GetEntityDescription: function GetEntityDescription(entity) {
        ...
        if (entity.Attributes[demoConfig.ProductDescriptionAttribute]) return entity.Attributes[demoConfig.ProductDescriptionAttribute][0];

        return "";
    },

    ReplaceImageUrl: function ReplaceImageUrl(entity) {
        ...
        for (var i = 0; i < demoConfig.ProductImageUrlAttributes.length; i++) {
            ...
            if (attr == "ExternalId") attrValue = entity.ExternalId;else if (entity.Attributes[demoConfig.ProductImageUrlAttributes[i]]) attrValue = entity.Attributes[demoConfig.ProductImageUrlAttributes[i]][0];
    ...
    InitFacetting: function InitFacetting() {
        ...
        for (var i = 0; i < demoConfig.Filters.length; i++) {
            $("div#filters").append($("<h2/>").html(demoConfig.Filters[i].Name)).append($("<div/>").attr("id", "filter_" + demoConfig.Filters[i].Name).addClass("filterdiv"));
        }
    },
    ...
    DisplayMore: function DisplayMore() {
        ...
        if (Demo.BottomVisible()) {
            ...
            if (Demo.PreviousSearch.TotalItems > (Demo.PreviousSearch.Page + 1) * demoConfig.DirectResultsPageSize) Demo.Search(Demo.PreviousSearch.Query, false, Demo.PreviousSearch.PreventReSearch, Demo.PreviousSearch.Page + 1);else if (Demo.PreviousSearch.TotalItems > demoConfig.DirectResultsPageSize && $("div#directresults div.endofresults").length == 0) {
    ...
    HashChanged: function HashChanged(previousHash, currentHash) {
        ...
        if (currentHash) {
            ...
            if (currentHash !== Demo.JustSetHash) {
                ...
                if (demoConfig == null || configName !== demoConfig.Name) Demo.LoadDemoConfig(configName, moveFunc);else moveFunc();
            }
        }
    },
    ...
    LoadDemoConfig: function LoadDemoConfig(configName, callback) {
